...
class PrnewsTest extends DuskTestCase
{
    ...
    {
        $url = $this::HOME_URL . '/sites/page/' . $pageId . '/perpage/96/';
    ...
    private function setTotalPages() : void
    {
        $this->counters['countPages'] = intval(
            $this->browser
                ->visit($this::HOME_URL . '/sites/page/3/perpage/96/')
                ->pause(1000)
                ->assertDontSeeIn('h1', 'Page not found')
                ->element('.pagination > li:nth-child(9) a')
                ->getText()
        );
    }
